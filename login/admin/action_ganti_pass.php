			...
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					if($update){
						...
						echo "<script>alert('Password berhasil di rubah');window.location='v_ganti_pass.php?pesan=oke';</script>";
					}else{
						//kondisi jika proses query gagal
						echo "<script>alert('Gagal merubah password');window.location='v_ganti_pass.php';</script>";
					}					
				}else{
					//kondisi jika password baru beda dengan konfirmasi password
					echo "<script>alert('Konfirmasi password tidak cocok');window.location='v_ganti_pass.php';</script>";
				}
			}else{
				//kondisi jika password baru yang dimasukkan kurang dari 5 karakter
				echo "<script>alert('Minimal password baru adalah 5 karakter');window.location='v_ganti_pass.php?pesan=oke';</script>";
			}
		}else{
			//kondisi jika password lama tidak cocok dengan data yang ada di database
			// echo 'Password lama tidak cocok';
			echo "<script>alert('Password lama tidak cocok');window.location='v_ganti_pass.php';</script>";
		}
	}
?>