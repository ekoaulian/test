<?php
    ...
    $sqlid = mysqli_query($koneksi, "SELECT id FROM artikel WHERE app_status = 2 order by id DESC LIMIT 1") ;
    $dataid = mysqli_fetch_assoc($sqlid);
    $genID = (empty($dataid)) ? 0 : $dataid['id'];
?> 

  <?php
    $data = mysqli_query($koneksi, "SELECT DISTINCT t.teknisi_name FROM artikel as me inner join toko as t on 
  ...
  <br/>
  <?php
      $data = mysqli_query($koneksi, "SELECT DISTINCT listrik_personil,tanggal FROM artikel where id = $genID");
      $d = mysqli_fetch_assoc($data);
  ?>
  ...
  <?php
    $data = mysqli_query($koneksi, "select count(listrik_id) as el_mon from artikel where listrik_status = 'unserviceable' and id = $genID") ;
    $d = mysqli_fetch_assoc($data);
    echo $d['el_mon'] ;
  ?>
  ...
  <?php
    $data = mysqli_query($koneksi, "select count(listrik_id) as el_mon from artikel 
  ...
  <?php
    $data = mysqli_query($koneksi, "SELECT DISTINCT listrik_personil,tanggal FROM artikel where id = $id");
    $d = mysqli_fetch_assoc($data);
  ?>
  ...
  <?php 
    $sql = "select e.listrik_name,me.listrik_status,me.listrik_keterangan from artikel as me inner join menu as e ON e.listrik_id = me.listrik_id where me.id = $id";
